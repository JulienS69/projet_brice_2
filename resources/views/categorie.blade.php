                                ...
                                @foreach($catégories as $categorie)
                                    <tr>
                                        ...
                                        <td class="text-truncate" style="max-width: 200px;"><a
                                                href="{{route('showArticleFromCateg' , ["id" => $categorie->id])}}">
                                                Voir les articles de cette catégorie </a></td>
                                        <td>{{$categorie->created_at->format("d/m/Y")}}</td>
                                        @if(Auth::user()->admin)
                                            <td class="text-center"><a class="btn" href="{{ route('addCateg') }}"><i
                                                        class="fa fa-plus-circle"></i></a></td>
                                            <form action="{{ route("delete_categ", ["id" => $categorie->id]) }}"
                                                  method="post">
                                                @csrf
                                                @method("DELETE")
                                            ...
                                            <form action="{{ route("showCateg", ["id" => $categorie->id]) }}"
                                                  method="post">
                                                @csrf
                                                <td class="text-center">
